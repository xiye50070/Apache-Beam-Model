...
if [ ! -d "$HADOOP_HOME/pid" ];then
        echo "mkdir $HADOOP_HOME/pid"
        mkdir "$HADOOP_HOME/pid"
fi

# export YARN_PID_DIR="$HADOOP_HOME/pid"
pid=$YARN_PID_DIR/yarn-$YARN_IDENT_STRING-$command.pid

# ResourceManagerNodeManagerJMX
export HADOOP_HISTORYSERVER_OPTS="$HADOOP_HISTORYSERVER_OPTS -javaagent:/opt/exporter/jmx/jmx_prometheus_javaagent-0.16.1.jar=9206:/opt/exporter/jmx/historyserver.yaml"
export YARN_TIMELINESERVER_OPTS="$YARN_TIMELINESERVER_OPTS -javaagent:/opt/exporter/jmx/jmx_prometheus_javaagent-0.16.1.jar=9207:/opt/exporter/jmx/timelineserver.yaml"
export YARN_RESOURCEMANAGER_OPTS="$YARN_RESOURCEMANAGER_OPTS -javaagent:/opt/exporter/jmx/jmx_prometheus_javaagent-0.16.1.jar=9204:/opt/exporter/jmx/resourcemanager.yaml"
export YARN_NODEMANAGER_OPTS="$YARN_NODEMANAGER_OPTS -javaagent:/opt/exporter/jmx/jmx_prometheus_javaagent-0.16.1.jar=9205:/opt/exporter/jmx/nodemanager.yaml"
